...
const UserVerificationFrom = () => {
  ...
  return (
    <form
      ...
      className={style.userVerificationForm}
    >
      <h1 className={style.userVerificationFormHeader}>
        Введите{' '}
        <span className={style.userVerificationFormHeaderSpan}>Email</span> или{' '}
        <span className={style.userVerificationFormHeaderSpan}>номер</span> для
        проверки пользователя
      </h1>
      <div className={style.userVerificationFormLabelInput}>
        <label htmlFor="email" className={style.userVerificationFormLabel}>
          E-mail:
        </label>
        <input
          type="email"
      ...
      <div className={style.userVerificationFormLabelInput}>
        <label htmlFor="number" className={style.userVerificationFormLabel}>
          Номер:
        </label>
        <NumberInput number={number} handleInput={handleInput} />
      </div>

      <button type="submit" className={style.userVerificationFormButton}>
        Проверить
      </button>

      {data && (
        <h4 className={style.UserVerificationFormDataText}>Результат:</h4>
      )}

      {data &&
        data.map((user, index) => (
          <div key={index} className={style.UserVerificationFormData}>
            <p className={style.UserVerificationFormDataText}>
              Email: {user.email}
            </p>
            <p className={style.UserVerificationFormDataText}>
              Номер: {user.number}
      ...
      {error && (
        <div className={style.UserVerificationFormDataError}>
